...
import { cn, formatTimeDifference } from './util';
...
describe('formatTimeDifference', () => {
  ...
  it.each([
    ...
    [24 * 60 * 60 * 1000, '1 day', 'exactly one day (24 hours)'],
    [25 * 60 * 60 * 1000, '1 day 1 hour', 'one day and one hour'],
...
describe('cn', () => {
  ...
  it('should filter out falsy values', () => {
    expect(cn('foo', '', 'bar', 0, null, 'baz', undefined)).toBe('foo bar baz');
  });
});
